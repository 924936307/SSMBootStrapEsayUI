...
<script type="text/javascript">
	...
	function refund(id){
		if(confirm("您确认要退票吗？")){
			$.post("${pageContext.request.contextPath}/TicketOrderSys/ticket/refund",{id:id},
				function(result){
					var result=eval('('+result+')');
					if(result.success){
						alert("退票成功！");
						window.location.href="${pageContext.request.contextPath}/TicketOrderSys/ticket/myTicketOrderList";
					}
				}
			);
		}
	...
	function search2(fromCity,id){
		...
		$.post("${pageContext.request.contextPath}/TicketOrderSys/flight/list2",{'s_flight.fromCity':fromCity,'s_flight.toCity':toCity},
				function(result){
	...
	function save(){
		...
		if(newFilghtId==undefined){
			...
			$.post("${pageContext.request.contextPath}/TicketOrderSys/ticket/meal",{'id':id,'newFlightId':newFilghtId},
					function(result){
						var result=eval('('+result+')');
						if(result.error){
...
<div class="panel panel-default">
  <div class="panel-heading" style="font-size:16px;">我的订单</div>
  <c:choose>
		  ...
		  <table class="table" >
		        ...
		        <tbody id="t2" style="text-align:center;">
				         <c:forEach var="ticketOrder" items="${ticketOrderList }">
				          <tr>
				            <td>${ticketOrder.orderno }</td>

				            <td>
				            	${ticketOrder.flightName }<br/>
				            	${ticketOrder.fromCity } - ${ticketOrder.toCity }<br/>
								${ticketOrder.fromTime }<br/>
								${ticketOrder.fromTime } - ${ticketOrder.toTime }<br/>
				            	<%--<fmt:formatDate value="${ticketOrder.fromTime }" type="date" /><br/>
				            	<fmt:formatDate value="${ticketOrder.fromTime }" type="date" /> - <fmt:formatDate value="${ticketOrder.flight.toTime }" type="date"/>--%>
				            </td>

				            <td>${ticketOrder.ordertime }</td>
  ...
  <div class="modal-dialog" >
    <div class="modal-content">
      ...
      <div class="modal-body">
      	  <div>
      	  	出发地点：<input type="text" id="fromCity"  name="fromCity"  style="width: 150px"/>&nbsp;&nbsp;
      	  	到达地点：<input type="text" id="toCity" name="toCity"  style="width: 150px"/>&nbsp;&nbsp;
